...
#-----------------------------------------------
# S3 Bucket Public Access Block (Static Assets)
#
# 静的アセットバケットのパブリックアクセスブロック設定
# - セキュリティ強化のため全てのパブリックアクセスをブロック
# - CloudFront OAC経由でのみアクセス可能（セキュアな構成）
# - この設定は本番環境でも維持推奨
#-----------------------------------------------
resource "aws_s3_bucket_public_access_block" "static_assets_pab" {
  bucket = aws_s3_bucket.static_assets.id

  # 全てのパブリックアクセスをブロック（CloudFront OAC使用のため安全）
  block_public_acls       = false  # パブリックACL作成をブロック
  block_public_policy     = false  # パブリックポリシー設定をブロック
