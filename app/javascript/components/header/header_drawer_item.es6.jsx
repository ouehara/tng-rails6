...
export default class HeaderDrawerItem extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      is_open: false, //default
      has_sub_pages: !!this.props.elements.items,
  ...
  renderTitleElement(is_link, data) {
    if (is_link) {
      return (
        <a
          href={typeof data.index == "undefined" ? data.url : data.index.url}
          className=""
          id={data.id}
  ...
  renderSubList(data) {
    ...
    return Object.keys(data).map((key, index) => {
      const the_item = data[key];
      if (this.state.is_pc) {
        return (
          <li className="sub_nav_item" key={index}>
            <a href={the_item.url} id={the_item.id}>
              {the_item.page_name}
            </a>
          </li>
        );
        ...
        return Object.keys(the_item).map((key_2, index_2) => {
          return (
            <li className="sub_nav_item" key={index_2}>
              <a href={the_item[key_2].url} id={the_item[key_2].id}>
  ...
  render() {
    const has_index_link =
      typeof this.props.elements.index == "undefined"
        ? this.props.elements.url
        : this.props.elements.index.url;
