...
resource "aws_ecs_task_definition" "rails_app_task" {
  ...
  cpu                      = "1024"  # 2048→1024 (コスト最適化: 開発環境用)
  memory                   = "2048"  # 8192→2048 (メモリ使用率7%以下のため大幅削減)
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        },
        {
          ...
          value = "13"  # Reduced for 1 vCPU (was 15)
        },
        {
          name  = "DB_POOL"
          value = "25"  # 2 workers × 10 threads = 20 connections + overhead
        },
        {
          name  = "WEB_CONCURRENCY"
          value = "2"  # Reduced to 2 workers for 1 vCPU (was 3)
        },
...
resource "aws_ecs_service" "rails_app_service" {
  ...
  health_check_grace_period_seconds = 120  # 2分 - マイグレーション時間を考慮
